...
2026-09-20：B 高清选档及校准/裁剪连续性以 [覆盖说明](COMMENTARY_VIDEO_OVERLAY.md#2026-09-20-校准连续性候选) 为准；已不再使用下文历史版本的最低画质及隐藏预览后关闭视频轨策略。Mac 三档解码见 [解码模式](MACOS_DECODE_MODES.md)。

2026-09-10 桌面卡顿候选另见 [排查记录](DESKTOP_PLAYBACK_STUTTER.md)。该历史批次 Mac 强制软件解码和软件输出；下文 `auto-copy` 是 3.0.7 历史配置，不能用来判断当前版本的硬解状态。
...
2. B 校准窗口是否能稳定渲染；反复校准→裁剪→覆盖时不能重新开流或关闭视频轨，偏移应保持。
...
3. 校准/裁剪在前台保持同一视频轨；后台视频资源管理单独验证，不能以牺牲已校准时间关系为代价。
