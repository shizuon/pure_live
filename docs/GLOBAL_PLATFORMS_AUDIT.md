# 国际直播平台接入（2026-09-21）

基线：fork `5f7b8b50180e45b6fb4d4d07f0aadd3109c49ec3`；冻结上游
`b97ed3ed628e6bf9dfacd96be3b4418da40eb966`。未合并上游。

## Twitch 遗漏：upstream-existing

用户样本 `croissantstrike`，StarLadder 决赛 / Counter-Strike。
公共 GraphQL 实测频道存在、正在直播、语言 `FR`、游戏 slug `counter-strike`。
原分区请求固定 `broadcasterLanguages: [ZH, KO]`，在接口请求时就排除了该频道。
另一个独立遗漏来自将 SearchCategoryTags 的 100 个推荐标签当作完整目录；
无标签或未入选标签的游戏无法进入 UI。改用无标签过滤的目录游标分页。
房间分页调用未传 pageSize，远端返回 30 条而 UI 只取所需数量，剩余条目被丢弃。
补齐 pageSize；刷新清理后续游标，避免新旧分页交叉；缺头像、游戏字段不再清空整页。
频道链接精确解析，合法登录名补充详情查询，不完全依赖搜索索引。

AngelLive `8cd8245df981d4fc0cc54e628da26b1772f00a6a` 当前主库通过外部插件源
加载平台，未找到可直接复用的 Twitch/Kick/YouTube 实现。本轮独立适配公共接口，
不引入其播放器实现。所有应用构建使用 GitHub Actions，平台串行。

验证需区分确定性适配器测试、公共接口探测、云构建及用户设备播放；
接口成功不代表所有地区、受限内容或长期播放均正常。回滚以本轮功能提交为单位。

## 3.0.23 候选支持范围

| 平台 | 发现和收藏 | 播放 | 弹幕 |
| --- | --- | --- | --- |
| Kick | 直播推荐、完整分类、分类直播、频道搜索、频道链接导入 | 公开直播 HLS，多画质，可选作 A/B | 动态读取公开 Pusher 配置，订阅成功才显示已连接，15 秒重试 |
| YouTube | 直播搜索、主题入口、视频链接及 @handle/channel 链接导入 | 公开直播 HLS，多画质，可选作 A/B | 公开直播聊天，按服务端间隔轮询，15 秒重连 |
| Twitch | 无语言限制的分类直播、无标签过滤的分类目录、登录名和链接精确查找 | 保留原播放实现 | 分类卡片使用 login 订阅，保留原 IRC 实现 |

YouTube 的主题入口是带直播过滤的搜索，不冒充完整平台分区目录。搜索只列正在直播的
视频，不包含普通录播和预告。视频链接收藏对应这场直播；频道链接收藏保留频道身份，
下次播放时重新找该频道当前直播。YouTube 不把 videoDetails.viewCount 的累计播放量
当成实时在线人数。Kick 搜索未给人数时留空。

无新增 SDK、外部服务器、yt-dlp/FFmpeg 子进程或媒体库替换。复用当前播放及双流流程，
包括高清 B 画质、校准、覆盖、A/B 弹幕选择及延迟、录制互斥。公开接口无法播放的
登录限定、会员、地区限制直播会报不可用；不实现登录和限制绕过。未开放聊天的
YouTube 房间明确显示未开放，不伪装连接成功，也不持续空轮询。

旧配置迁移仅加入两个新平台，不重新开启用户隐藏的旧平台。平台名称不变；
图标取自各自官网公开的 apple-touch-icon/favicon，仅用于标识对应平台，商标归原权利人。

## 外部接口证据

2026-09-21 匿名探测：

- Twitch：`croissantstrike` / `FR` / `counter-strike`；ChannelShell 和 StreamMetadata
  返回用户和 live 状态，primaryTeam 非关键字段偶发 service error，不阻止读房间。
  无标签目录首屏返回 100 项、包含 Counter-Strike 并提供下一页游标。
  最终 `DirectoryPage_Game` 无语言过滤、默认 limit 30 的实际请求返回 29 个直播间，
  第一页包含 `croissantstrike`，`hasNextPage=true`，无 GraphQL errors。首次额外探测
  遇到连接重置，重试取得上述结果；未将临时传输失败当成代码遗漏。
- Kick：搜索、分类和直播目录可用；直播目录至少返回 5 条，因此适配器缓存超额条目。
  在线房间 HLS 返回 5 档，含音轨。`web.kick.com/api/v1/realtime/channels/.../chat/connection`
  返回公开 Pusher 连接参数。旧固定实时连接地址不作为配置来源。
- YouTube：网页读取当前视频和客户端上下文；ANDROID 播放请求返回 HLS，清单可访问、
  共 6 档且含音轨。WEB/IOS 不一定返回 HLS，因此不会把其 adaptiveFormats 中的
  视频单轨 URL 当完整直播流。开放聊天的房间续页实测返回 73 条 action。
- 真实响应仅保留在本机忽略目录用于 Dart 回放测试，不提交签名播放 URL、聊天续页
  标识或访问者上下文。测试用例使用独立构造的数据。

设备验证尚未完成；当前证据不代表已在手机或桌面完成实际连续播放。

## 本轮验证与交付

- Flutter 3.47.0，完整静态分析通过（0 error）；新增跨适配器 HLS 调用的注解告警及
  多行条件格式已修正，忽略目录中的旧排查脚本 lint 不属于发布源码。
- 完整测试首轮 `549 passed / 1 failed`，唯一失败是录制契约旧测试要求平台总数 10。
  更新为 12 后，录制契约、新平台、Twitch、媒体请求头和真实响应回放共 29 项通过。
  本轮没有改动媒体库、软硬解设置或双流时钟算法。
- 新增确定性测试覆盖：法语主播可见、完整无标签目录、刷新清理游标、精确频道查询、
  Kick 多余分页结果保留、YouTube 直播过滤与分页、频道收藏身份、设置迁移幂等、
  聊天重连、未开放聊天、退出时取消定时器、晚到响应隔离、回调中退出。
- 应用构建仅在 GitHub；Mac → Android → iOS 串行。iOS 提供普通未签名 IPA，
  由用户自行签名。不操作本机播放器或连接手机。构建成功和设备播放仍分开记录。
- 额外仓库完整性审计报 18 条历史规则违规，均位于本轮未修改、未使用的
  `build_pure_live_release.yml`（可变依赖引用、未锁定 pub get、默认开启多平台等）；
  本轮使用固定引用、默认关闭平台的 `feature-build.yml`，不把这次审计称为全仓通过。
- 最终真实 Dart 联网探测补充：Kick 复用 Twitch 的 Chrome/137 UA 时，目录、频道详情、
  分类均返回 403；相同 Dio 请求使用 `Mozilla/5.0` 时三者均为 200。已独立设置 Kick
  接口、弹幕配置和媒体请求头。完整 Kick 目录 → 详情 → HLS → 搜索 → Pusher 订阅
  实测通过；YouTube 直播搜索 → 频道直播 → HLS → 聊天续页握手实测通过。测试结束
  均主动关闭连接。该证据仍不是原生解码、用户地区网络或长时运行验证。
- 普通 11 字符搜索词不再误作 YouTube 视频 ID；新增回归后新平台 18 项通过。
  最终定向分析为 0 error / 0 warning，仅 2 条 null-aware 风格提示。
- Kick 分类筛选实测要求 slug，数字 ID 会被服务端静默忽略并返回全站列表。
  已传递 `shortName`，以 `counter-strike-2` 实测返回 StarLadder、MAJ3R、Gaules 等
  CS 直播，补充对应请求契约测试。

## 云端交付进度

- 最终源码 `e46fc641a794f37e840c987d53fb7afec416ea52` 已推送用户仓库 master 与
  `feat/global-platforms-20260921`。主分支一并包含之前移动端、高清 B 和校准修复。
- Mac [运行 35526922008](https://github.com/shizuon/pure_live/actions/runs/35526922008)
  已成功；下载后确认版本 `3.0.23+4111`，主程序含 `x86_64 arm64`，ZIP CRC、
  DMG `hdiutil verify` 及两个包的 SHA-256 均通过。仅检查包内容，没有安装或运行应用。
  此前两个候选运行已取消，不作为交付证据。
- Mac [下载附件（含 DMG 和 ZIP）](https://github.com/shizuon/pure_live/actions/runs/35526922008/artifacts/10610735959)，
  GitHub 标记有效期至 `2026-09-23 18:18:35 UTC`，下载通常需要登录 GitHub。
  外层附件 ZIP 的 SHA-256 与 GitHub 返回的 digest 一致：
  `2c1d1a8551314073dc6378121396e0b39d0dbed3a894aab31fcd96e2b91fbf17`。
- Android [运行 35530227640](https://github.com/shizuon/pure_live/actions/runs/35530227640)
  已成功并下载核验。源码 `d192b717c20ee2596430941af5552b8e1fcd7f95`，
  相对 Mac 源码仅增加审计文档，业务源码一致。仅启用 Android arm64 测试签名，
  不创建正式 Release。云端内容检查通过（1261 个 Flutter 资源），签名 v2 验证通过；
  manifest 为 `3.0.23`、versionCode `6111`（Flutter build `4111` 加 arm64 偏移 `2000`）。
  本机下载后 APK SHA-256 与云端校验文件一致，ZIP CRC、版本资源和唯一 ABI
  `arm64-v8a` 均通过，没有安装或操作设备。
- Android [下载测试 APK](https://github.com/shizuon/pure_live/actions/runs/35530227640/artifacts/10611510440)，
  GitHub 标记有效期至 `2026-09-23 18:59:05 UTC`。使用本次构建临时测试证书，
  证书 SHA-256 为 `2dbe4c825bcb06569fc8812c24052108d9b875a76d0aae0c4c96f067c720ba21`；
  与已安装版本签名不同时不能直接覆盖升级，请先备份设置，不把它当固定证书正式包。
- iOS [运行 35531634781](https://github.com/shizuon/pure_live/actions/runs/35531634781)
  已成功并下载核验。构建 ref 为 `feat/global-platforms-20260921`，
  实际提交 `a453e090315718a6f92102aaebf0a592813a0247`；相对 Mac 仅文档变化。
  云端明确关闭 codesigning；交付普通未签名 IPA，供用户自行签名，不以 TrollStore 包代替。
  本机 SHA-256 与云端校验文件一致，版本为 `3.0.23+4111`，包名为
  `com.mystyle.purelive.liu`；`tool/verify_unsigned_ipa.py` 验证 ZIP CRC、iPhoneOS
  标识、主程序及 Flutter/App 框架 arm64 Mach-O、Flutter 资源均通过。
- iOS [下载普通未签名 IPA](https://github.com/shizuon/pure_live/actions/runs/35531634781/artifacts/10611856587)，
  GitHub 标记有效期至 `2026-09-27 19:29:07 UTC`。签名、描述文件和实机安装由用户处理；
  包结构验证不等于 Apple 签名验证，也不保证所用签名渠道和设备可安装。
- 三平台串行云构建、下载及包核验已完成；没有创建正式 Release，没有本地编译或操作设备。
  本轮完成后停用 `pure-live-3-0-23` 自动跟进。全部平台仍缺少设备播放验证，
  尤其需用户确认新增平台的持续播放、A/B 校准、画面覆盖、弹幕切换及前后台行为。

| 已核验产物 | 字节数 | SHA-256 |
| --- | ---: | --- |
| `PureLive-3.0.23-4111-macos-universal.dmg` | 122468086 | `0c7d4681ded8ce0f6b3f74cfac8a3edba809c9a2947f4243e3390b9373d8ae09` |
| `PureLive-3.0.23-4111-macos-universal.zip` | 106598282 | `1bbc79dcf8e0ff9eaee3fb6d2217b1bcc9793292f4835e40979ffd520fd69709` |
| `PureLive-3.0.23-4111-android-arm64-v8a-test-signed.apk` | 118731207 | `355d8d1c39bff1d67a2bb88dd36374844d86b456653ca17ecea72682de91b8bb` |
| `PureLive-3.0.23-4111-ios-arm64-unsigned.ipa` | 60462528 | `4df5123c0169f128835a8a6a9057e7f8bc40f7eee1871d08b4cb8bc6ebfeeff3` |

Mac 附件最初经 `gh run download` 两次遇到连接重置；通过 GitHub API 取得同一附件地址，
使用 curl 重试后完整下载，外层哈希和内层校验均一致。不需要修改源码或重新构建。
本机忽略目录保留下载产物：`local-artifacts/cloud-global-macos-3.0.23/`。
Android 下载产物保留于 `local-artifacts/cloud-global-android-3.0.23/`，
云端验证日志保留于 `local-artifacts/global-platforms-android-cloud.log`。
iOS 下载产物保留于 `local-artifacts/cloud-global-ios-3.0.23/`，
云端验证日志保留于 `local-artifacts/global-platforms-ios-cloud.log`。
