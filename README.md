...
`3.1.0` 统一整理网页登录、账号退出和实际画质显示。B站保留扫码，其他已有账号的平台优先打开官方登录页，手动 Cookie 收进高级选项；Linux 缺少当前网页组件，只保留B站扫码和高级入口。各客户端云构建与验证状态见[3.1.0交付记录](docs/PLATFORM_LOGIN_3_1_0.md)，登录不保证平台开放所有画质。
