# Pure Live v3.0.20 Mac 解码测试候选

- Mac 新增「软解」「硬解 only」「硬解兼容」。默认保持软解；已有播放器时修改需完全退出并重启，A/B 和多画面使用同一档。
- 硬解 only 使用 VideoToolbox＋硬件输出，不继续软解播放；硬解兼容使用 VideoToolbox 回拷＋现有软件输出，允许失败后回退。
- 新增 A/B 实际解码状态手动读取；B 纯音频不误报硬解失败。
- 保留 HDR 合成兼容设置、视口尺寸优化、双流校准、裁剪覆盖及 3.0.19 全部修复。其他平台解码行为不变，媒体库依赖不变。
- 本版是 Mac 实机测试候选，尚未证明 HDR 无闪烁或 CPU／帧率改善。操作与验证说明：[Mac 三档解码测试](docs/MACOS_DECODE_MODES.md)。

## Pure Live v3.0.19 清理测试候选

- 移除额外的设置 `onReady()` 排队调度，统一 IPTV 设置注册。
- 桌面 FFmpeg 按录制/处理需要初始化；直播间录制按钮只读取轻量任务状态，打开菜单不启动录制服务。
- 无写入任务或关闭缓存限制时停止周期巡检；仍在明确使用、开启限制和录制完成时检查已有文件。
- B 从最低画质开始按需解析，同档线路失败后才解析下一档；重新同步仍刷新签名，切换 B 后丢弃旧请求结果。
- 修复 B 等待稳定播放时快速切换解说源，旧任务可能误释放新播放器的问题。
- 保留 3.0.18 的桌面卡顿候选、斗鱼弹幕重连、双流校准和裁剪覆盖修复。调用/任务减少已有定向测试证据，实际 CPU、内存和首录表现仍需安装包对照。见 [清理实验记录](docs/CLEANUP_ABLATION_RESULTS.md)。

---

# Pure Live v3.0.18 测试候选

- 修正 Windows 视频输出在 GPU 完成超过一个渲染间隔时，持续丢失待完成帧跟踪、无法交付新画面的状态错误。
- macOS 普通适应窗口的画面按实际显示像素生成软件输出，减少小窗口的图像转换开销；全屏保留源分辨率，HDR 兼容配置不变。
- 隐藏主画面弹幕时卸载绘制引擎，停止不可见的弹幕推进；弹幕连接和历史列表保持独立。
- 保留 v3.0.17 的斗鱼切房后弹幕重连修复，以及双流、再次校准、九点裁剪覆盖和退出清理。
- Windows 副屏与 macOS 后台卡顿仍需候选包实测；尚未确认偶发卡顿是否还包含线路缓冲问题。证据与限制见 [排查记录](docs/DESKTOP_PLAYBACK_STUTTER.md)。
...
- 支持使用 A 直播间画面和 B 直播间声音，并提供 10/100/500 ms 手动校准与漂移修正。
- 双流模式可在 A/B 之间明确选择弹幕来源；B 弹幕复用视频偏移。
- 解说源失效时恢复 A 原声；双流期间禁止组合音画录制。
